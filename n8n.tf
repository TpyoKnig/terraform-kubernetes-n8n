...
check "rolling_update_still_overlaps_two_mains" {
  assert {
    ...
    error_message = "${local.n8n_main_strategy_left_to_kubernetes ? "n8n_extra_helm_values deletes the strategy key, and in Helm an explicit null is a deletion rather than an absence, so the main Deployment falls back to Kubernetes' own RollingUpdate at maxSurge 25%. That surges a second main to Ready beside the outgoing one." : "The main Deployment is set to roll with RollingUpdate."} RollingUpdate does not give the main pod an overlap-free rollout: at best, when this module renders it, maxSurge = 0 means a second main is never scheduled, but the outgoing pod goes on running after the incoming one is created, for as long as it takes to terminate: the kubelet starts the pod's terminationGracePeriodSeconds clock when the pod is marked for deletion, runs the preStop hook inside that budget, and only then sends SIGTERM, so the whole window is bounded by that one number rather than by any sum, give or take the one-off two-second extension the kubelet grants if the hook is still running when the budget expires. For that window two n8n mains share one Redis and one Postgres with no leader election between them, because multi-main is licensed and this module deploys Community edition: both hold the schedule triggers, cron workflows fire twice, and on a version bump the new main runs its one-way startup migrations while the old one is still reading the old schema. Set n8n_main_strategy = \"Recreate\" unless your tooling requires the RollingUpdate strategy type and you accept that window. If this fired on an apparently default configuration, check n8n_extra_helm_values: the overlay is merged last and wins, and in Helm an explicit null is a deletion, so strategy: null or strategy.type: null there removes this module's Recreate and leaves Kubernetes' own RollingUpdate default at maxSurge 25%, which surges a second main rather than merely overlapping a terminating one."
  }
}
