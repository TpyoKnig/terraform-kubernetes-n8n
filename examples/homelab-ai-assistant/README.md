...
`$API_KEY` is the value the n8n side needs next - keep it. `fullnameOverride=sandbox` is not cosmetic: without it the chart's fullname template concatenates release and chart name (release `sandbox` does not contain `n8n-sandbox-service`, so nothing is dropped) and the API Service comes out `sandbox-n8n-sandbox-service-api`. This example's `sandbox_release_name` variable builds `N8N_SANDBOX_SERVICE_URL` as `<release>-api`, which is only true because the override pins the fullname to the release name. Change the release name and the override together, and mirror both in `terraform.tfvars`; get it wrong and the assistant loads normally while every code execution times out against a Service that does not exist.
