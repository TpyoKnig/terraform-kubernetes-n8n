...
# Plan-time tests for the homelab-ai-assistant example using mocked providers.
#
...
#
# Run: terraform test
#   (from examples/homelab-ai-assistant/, mocks require terraform >= 1.11)
...
# ── ai_assistant output ──────────────────────────────────────────────────────
# helm_release.values resolves to (known after apply) under the mocked
# providers (see the file header), so what actually reaches
# config.extraEnv in the n8n release is not assertable here - that coverage is
# the module's own tests/defaults.tftest.hcl. What this root declares itself is
# the ai_assistant output's construction from sandbox_namespace and
# sandbox_release_name, which is plain interpolation this test can reach.

run "sandbox_service_url_uses_the_defaults" {
  command = plan

  assert {
    condition     = output.ai_assistant.sandbox_service_url == "http://sandbox-api.n8n-sandbox.svc.cluster.local:8080"
    error_message = "Default sandbox_release_name \"sandbox\" + sandbox_namespace \"n8n-sandbox\" should render http://sandbox-api.n8n-sandbox.svc.cluster.local:8080; got ${output.ai_assistant.sandbox_service_url}."
  }

  assert {
    condition     = output.ai_assistant.model == "anthropic/claude-opus-4-8"
    error_message = "ai_model default should pass through to the output unchanged; got ${output.ai_assistant.model}."
  }
}

run "sandbox_service_url_follows_a_non_default_release_and_namespace" {
  command = plan

  variables {
    sandbox_namespace    = "sandbox-ns"
    sandbox_release_name = "my-sandbox"
    ai_model             = "openai/gpt-5"
  }

  assert {
    condition     = output.ai_assistant.sandbox_service_url == "http://my-sandbox-api.sandbox-ns.svc.cluster.local:8080"
    error_message = "sandbox_service_url must follow both sandbox_release_name and sandbox_namespace; got ${output.ai_assistant.sandbox_service_url}."
  }

  assert {
    condition     = output.ai_assistant.model == "openai/gpt-5"
    error_message = "ai_model should pass through to the output unchanged; got ${output.ai_assistant.model}."
  }
}
