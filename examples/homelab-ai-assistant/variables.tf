...
variable "ai_model" {
  description = "Provider-qualified model the AI Assistant and Agents module call, e.g. \"anthropic/claude-opus-4-8\". n8n reads this verbatim and does not validate it against a model list, so a typo or a retired model surfaces as the assistant failing every request rather than as a plan-time or startup error."
  type        = string
...
variable "sandbox_namespace" {
  description = "Namespace the n8n-sandbox-service Helm release runs in. This example does not create that release (see README) - it only builds N8N_SANDBOX_SERVICE_URL from this and sandbox_release_name, so the value has to match wherever you actually installed it."
  type        = string
  default     = "n8n-sandbox"
  nullable    = false
}
