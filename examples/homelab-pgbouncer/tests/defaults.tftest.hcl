...
# Plan-time tests for the homelab-pgbouncer example using mocked providers.
#
...
#
# Run: terraform test
#   (from examples/homelab-pgbouncer/, mocks require terraform >= 1.11)
...
run "postgres_host_names_the_pooler_not_the_cluster" {
  command = plan

  # This is the assertion that separates this example from ../homelab, where
  # the same output resolves to "<release>-pg-rw". With cnpg_pooler_enabled the
  # module points n8n at "<cluster>-pooler-rw" instead, and it does so in one
  # place (local.k8s_pg_host), so this output moving is the evidence that every
  # consumer of it moved too.
  assert {
    condition     = can(regex("-pg-pooler-rw", module.n8n.backing_services.postgres_host))
    error_message = "backing_services.postgres_host should reference the PgBouncer Pooler Service when cnpg_pooler_enabled = true; got: ${module.n8n.backing_services.postgres_host}"
  }

  # And specifically not the rw Service. A regex for the pooler name alone
  # would still pass if the host were "n8n-pg-rw" and the pooler suffix arrived
  # from somewhere unrelated, so pin the negative too.
  assert {
    condition     = !startswith(module.n8n.backing_services.postgres_host, "n8n-pg-rw.")
    error_message = "postgres_host must not be the CNPG rw Service when a pooler is enabled; got: ${module.n8n.backing_services.postgres_host}"
  }
}

run "the_direct_postgres_endpoint_stays_reachable" {
  command = plan

  # Transaction-mode pooling drops session state, so anything needing it
  # (session advisory locks, LISTEN/NOTIFY, an interactive psql) has to be able
  # to bypass PgBouncer. If this output ever follows postgres_host to the
  # pooler, that escape route is gone and the failure shows up during a
  # migration rather than here.
  assert {
    condition     = can(regex("-pg-rw", module.n8n.backing_services.postgres_direct_host))
    error_message = "backing_services.postgres_direct_host must still name the CNPG rw Service so session-scoped work can bypass the pooler; got: ${coalesce(module.n8n.backing_services.postgres_direct_host, "<null>")}"
  }
}
