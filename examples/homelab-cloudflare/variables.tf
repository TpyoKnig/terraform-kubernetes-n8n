...
variable "ui_host" {
  description = "Hostname served by ingress. This example creates the DNS record for it, a proxied CNAME to the Cloudflare Tunnel named by cloudflare_tunnel_id, when cloudflare_zone_id is set. Leave that null and this example behaves exactly like examples/homelab: the record is yours."
  type        = string
...
variable "cloudflare_zone_id" {
  description = "Cloudflare zone ID for the zone ui_host sits under. Leave null (the default) to manage DNS yourself - the example creates no record then. Set it and the example creates a proxied CNAME for ui_host pointing at the Cloudflare Tunnel named by cloudflare_tunnel_id. The API token needs Zone:DNS:Edit on this zone."
  type        = string
  default     = null

  validation {
    condition     = var.cloudflare_zone_id == null || var.cloudflare_tunnel_id != null
    error_message = "cloudflare_zone_id requires cloudflare_tunnel_id: the record is a CNAME to <tunnel-id>.cfargotunnel.com, so there is nothing to point it at otherwise. Find it with `cloudflared tunnel list` or in the Cloudflare Zero Trust dashboard."
  }
}

variable "cloudflare_tunnel_id" {
  description = "UUID of the Cloudflare Tunnel that fronts the cluster's ingress controller. The DNS record created when cloudflare_zone_id is set is a proxied CNAME to <this>.cfargotunnel.com. The tunnel itself is not managed by this example: it is long-lived cluster infrastructure serving every other hostname too, so a terraform destroy here must not be able to take it down."
  type        = string
  default     = null
}
