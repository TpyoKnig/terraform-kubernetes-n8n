...
output "n8n_oauth_callback_url" {
  description = "Redirect URI to register with any OAuth2 provider a credential will use (Slack, Google, Microsoft and the rest). n8n builds this from N8N_EDITOR_BASE_URL, which is the editor hostname, not the webhook one: on a split ingress the two differ, and this is the one n8n actually advertises: register the webhook hostname with a provider instead and the consent flow redirects to a name n8n never issued this URI for. Note the split-ingress examples do route /rest on the webhook hostname for the Agents chat integrations, so that path answers there too; it answering is not the same as it being the advertised callback, and this output is the advertised one. Exposed because that is not guessable from the other outputs and getting it wrong fails in the provider's UI, at the end of a consent flow, with no n8n log line to find. The /rest segment is n8n's default and stays that way: N8N_ENDPOINT_REST is reserved by the module, so this cannot drift from what the workload serves."
