...
terraform {
  ...
  required_providers {
    ...
    kubectl = {
      ...
      # arrive on an unrelated apply, and this provider owns every custom
      # resource the module writes: the CNPG Cluster, the Pooler, and the
      # ClusterIssuer in modules/tls-letsencrypt.
      version = "~> 1.14"
    }
    time = {
