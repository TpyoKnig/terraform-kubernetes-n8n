...
variable "n8n_task_runner_auto_shutdown_timeout" {
  description = "Seconds of inactivity before an idle task-runner process exits. Maps to taskRunners.launcher.autoShutdownTimeout, which the chart renders into the task-runner SIDECAR's environment as N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT — not into the n8n containers, where the launcher does not run. Set to 0 to disable idle shutdown entirely: the launcher checks `idleTimeout === 0` and skips arming its timer. The trade is a resident runner process against cold-start latency, and the cold start is not small — a Code node that takes ~17ms on a warm runner takes several seconds on a cold one, because the launcher has to start the Node or Python process before the task can run. Leave the default if runner memory matters more than first-request latency; set 0 for latency-sensitive workflows that fire intermittently."
  type        = number
  default     = 15
}
