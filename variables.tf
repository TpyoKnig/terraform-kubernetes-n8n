...
variable "cnpg_postgres_image_tag" {
  description = "Postgres image tag for CNPG (ghcr.io/cloudnative-pg/postgresql:<tag>). Only used when postgres_backend = \"cnpg\". Defaults to the major version alone, which is a rolling tag: whatever minor it points at is what a newly created instance pulls. Note what that does and does not do. It does not roll a running cluster, because the tag string does not change and CloudNativePG has no new image reference to act on; the newer minor arrives whenever an instance is next recreated for some other reason. Nor is even that guaranteed, since a rolling tag resolves at pull time and the default pull policy for a non-latest tag is IfNotPresent: an instance recreated on a node that already cached this tag keeps the minor that node cached, so two instances of the same cluster can sit on different minors. To take a minor deliberately, bump this to the version you want, which does change the Cluster spec and does trigger CNPG's controlled rolling restart, or drive it from an operator ImageCatalog instead. That this is not pinned the way n8n_image_tag is, is on purpose: a PostgreSQL minor is security and bug fixes only and is data and wire compatible with the rest of its major, while an n8n upgrade runs one-way schema migrations. Upstream also publishes tags carrying the image type and distribution, `16.10-minimal-trixie` and the like, and documents those as the supported form; the bare `16` and `16.10` spellings are deprecated and are to be removed when the bullseye images reach end of life, so a qualified tag is the more durable choice even though the default here is still the bare major. Changing the MAJOR version is a different matter entirely, and not one this module guards. Current CloudNativePG treats a higher major declared here as a request for an offline in-place upgrade: it shuts down every pod in the cluster, replicas included, runs pg_upgrade in a job, and brings the cluster back on the new major. The database is unavailable for the whole of it, so this is not an edit to make casually from tfvars. If the job fails, reverting this input to the previous major lets the operator drop the failed job and restart on the original version, and no data is modified in the attempt. Operators old enough to lack that path, and anyone wanting the upgrade online, need the blue/green route instead: a second Cluster and either a dump/restore or logical replication."
  type        = string
  default     = "16"
  nullable    = false
