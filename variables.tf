...
variable "n8n_proxy_hops" {
  description = "How many proxies append to X-Forwarded-For between the client and an n8n pod, wired to N8N_PROXY_HOPS on every pod type. Defaults to 1, which counts a single ingress controller and is right for the Ingress this module renders. Raise it for anything further out: a Cloudflare Tunnel, a CDN, a WAF or an outer reverse proxy each add one, so ingress-nginx behind Cloudflare is 2. A wrong count is as bad as none, and nothing errors either way: too low and n8n reads a proxy's address as the client, too high and it reads a value the client could have forged, and everything keyed on the client address — Express's req.ip, and any rate limit or IP-based restriction built on it — depends on which. n8n writes no per-request source IP to its logs or audit events, so a wrong count never surfaces in n8n's own output; it shows only in whatever downstream consumer trusted the address. Set 0 only if nothing proxies to the pods at all. Rendered whether or not the module owns the Ingress, because a caller running their own routing (create_ingress = false) is behind a proxy just the same; that path previously emitted nothing, so n8n trusted no forwarded header and attributed every request to the ingress controller."
  type        = number
  default     = 1
  nullable    = false
...
variable "n8n_termination_grace_period" {
  description = "Seconds n8n is given after SIGTERM to finish in-flight work before the kubelet force-kills the pod. MINIMUM: do not lower below 60. Workers need time to finish in-flight executions before being terminated. This sets two things that have to agree, on main, worker and webhook processor alike: N8N_GRACEFUL_SHUTDOWN_TIMEOUT, which is how long n8n believes it has, and the pod's own terminationGracePeriodSeconds, which is when the kubelet stops asking. The pod's period is this value plus n8n_prestop_sleep, because the two clocks do not start together: Kubernetes begins the grace period when the pod is marked for deletion, runs the preStop hook, and only sends SIGTERM once that hook returns, so n8n's budget starts one drain sleep late and the pod needs the sum for this number to be honoured. The two-clocks wiring has been observed live: with a 20-second sleep and a 120-second budget, all three Deployments rendered terminationGracePeriodSeconds = 140, and SIGTERM landed 20.38 seconds after the delete request — after the hook returned, not alongside it."
  type        = number
  default     = 60
...
variable "n8n_prestop_sleep" {
  description = "Seconds the preStop hook sleeps before SIGTERM is sent, giving the ingress controller and the cluster's proxy time to remove the pod from their endpoint lists while it is still serving. Without the gap a rollout drops requests that were routed microseconds before the pod left the Service, which shows up as occasional 502s during an otherwise clean deploy. Rendered as the chart's lifecycle preStop command on main, worker and webhook processor. It also extends the pod's terminationGracePeriodSeconds, because SIGTERM is not sent until this hook returns and n8n's own shutdown budget only starts then; see n8n_termination_grace_period. MINIMUM: do not lower below 10."
  type        = number
  default     = 10
  nullable    = false
...
variable "cnpg_postgres_image_tag" {
  description = "Postgres image tag for CNPG (ghcr.io/cloudnative-pg/postgresql:<tag>). Only used when postgres_backend = \"cnpg\". Defaults to \"18-minimal-trixie\", the supported upstream spelling of a rolling major (the major version, the image type, and the Debian release the image is built on) on the PostgreSQL major current n8n supports. An existing cluster created on this module's previous default (the bare `16`) must pin that tag here BEFORE taking a module version with this default, then plan the move to 18 as its own change: letting the new default reach a running cluster requests a major upgrade the in-place path cannot serve, since this jump also crosses Debian releases (see the changelog entry for this default). Whatever minor that tag points at is what a newly created instance pulls. Note what that does and does not do. It does not roll a running cluster, because the tag string does not change and CloudNativePG has no new image reference to act on; the newer minor arrives whenever an instance is next recreated for some other reason. Nor is even that guaranteed, since a rolling tag resolves at pull time and the default pull policy for a non-latest tag is IfNotPresent: an instance recreated on a node that already cached this tag keeps the minor that node cached, so two instances of the same cluster can sit on different minors. To take a minor deliberately, bump this to the version you want, which does change the Cluster spec and does trigger CNPG's controlled rolling restart, or drive it from an operator ImageCatalog instead. Leaving this unpinned while n8n_image_tag is pinned is deliberate: a PostgreSQL minor is security and bug fixes only and is data and wire compatible with the rest of its major, while an n8n upgrade runs one-way schema migrations. The bare `16` and `16.10` spellings are still accepted and still resolve, but upstream has deprecated them for removal when the bullseye images reach end of life, and they name the `system` images, the one type that still ships the Barman Cloud binaries. The minimal and standard images do not, so the in-tree `cnpg_backup` form (spec.backup.barmanObjectStore) cannot archive WAL from them: that pairing needs either a deprecated system tag or the Barman Cloud Plugin wired through `cnpg_plugins`, and a plan-time check warns when `cnpg_backup` is set against an image type that cannot serve it. Changing the MAJOR version is a different matter entirely, and not one this module guards. Current CloudNativePG treats a higher major declared here as a request for an offline in-place upgrade: it shuts down every pod in the cluster, replicas included, runs pg_upgrade in a job, and brings the cluster back on the new major. The database is unavailable for the whole of it, so this is not an edit to make casually from tfvars. If the job fails, reverting this input to the previous major lets the operator drop the failed job and restart on the original version, and no data is modified in the attempt. Both halves of that have been exercised on a real cluster: letting this default reach a cluster created on `16` launched the in-place upgrade job and it failed exactly as described — the trixie-based job pod cannot execute the old bullseye binaries (libssl.so.1.1 missing), before pg_upgrade could even compare catalogs — and reverting the tag brought the cluster back healthy in under a minute with the data intact. It is also conditional: CNPG supports the in-place path only between images built on the same operating system distribution, so a bump that also crosses from bullseye to bookworm or trixie is not this upgrade whatever the version numbers say, and the extensions on both sides being compatible is your responsibility rather than the operator's. Take a backup first and rehearse it somewhere that does not matter. Operators old enough to lack the in-place path, anyone crossing distributions, and anyone who cannot take the downtime, need the blue/green route instead: a second Cluster and either a dump/restore or logical replication."
  type        = string
  default     = "18-minimal-trixie"
  nullable    = false
...
variable "cnpg_backup" {
  description = "The CloudNativePG Cluster's `spec.backup`, passed through verbatim. Null (the default) writes no backup stanza, which is what this module has always produced: a database whose only copy is its PVC, with `terraform destroy` and a StorageClass reclaim policy of Delete between you and every workflow, credential and execution it holds. Setting it turns on continuous WAL archiving to the object store you name, which is the half that gives point-in-time recovery. Note that a base backup still needs a `Backup` or `ScheduledBackup` custom resource, which is a separate object outside this module; docs/operations.md has a worked example of both halves. This is a passthrough rather than a typed input, deliberately and unlike every other input here, because the field's own shape has changed under CloudNativePG and a typed surface would have frozen one version of it. Terraform still checks the outer shape at plan, rejecting anything but null or a non-empty object; the trade is that contents the CNPG schema rejects surface only at apply, from the operator's webhook. Note the limit of what it reaches: CloudNativePG deprecated `spec.backup.barmanObjectStore` in 1.26 in favour of the Barman Cloud Plugin, and the plugin is configured through the Cluster's `spec.plugins`, which `cnpg_plugins` reaches, plus a separate `ObjectStore` resource that stays yours to create. The in-tree form this input writes still works on 1.26 and later, but only against an image that carries the Barman Cloud binaries, which today means the deprecated bare-tag `system` images: the minimal and standard images ship without them, the default image here is minimal, and a plan-time check warns about that pairing. A cluster that has moved to the plugin cannot be configured through here at all. Only used when postgres_backend = \"cnpg\"; on the external path it is silently inert, which a plan-time check warns about."
  type        = any
  default     = null
...
variable "cnpg_plugins" {
  description = "The CloudNativePG Cluster's `spec.plugins`, passed through verbatim: a list of CNPG-I plugin declarations, of which the Barman Cloud Plugin is the one this module documents (docs/operations.md has the worked example, including the `ObjectStore` resource the plugin reads, which stays yours to create alongside its credentials Secret). Null (the default) writes no plugins stanza. A passthrough for the same reason `cnpg_backup` is one: the plugin interface is where CloudNativePG's backup surface is actively moving, and a typed input would freeze one snapshot of it. The trade is the same too: Terraform checks only the outer shape at plan (null, or a non-empty list of entries that each carry a name), and contents the CNPG schema rejects surface only at apply, from the operator's webhook. The Barman Cloud Plugin needs CloudNativePG 1.26 or newer, and the plugin itself must be installed in the operator's namespace before any Cluster declares it; an operator too old to know `spec.plugins` prunes the field silently, the same failure shape cnpg_pdb_enabled documents. Do not declare a WAL-archiving plugin and a `cnpg_backup` carrying barmanObjectStore on the same cluster: both would claim the archive command, and which one wins is not yours to choose; a plan-time check warns about the combination. Only used when postgres_backend = \"cnpg\"; on the external path it is silently inert, which a plan-time check warns about."
  type        = any
  default     = null

  validation {
    # A tuple/list whose entries each carry the name the operator dispatches
    # on. Anything else is malformed at a level worth naming here rather than
    # leaving to Terraform's generic type mismatch. can(var.cnpg_plugins[0])
    # holds only for a non-empty sequence: indexing errors on a map, a string,
    # a lone object and the empty list alike, and an empty list would render
    # `plugins: []`, a stanza that claims plugins and names none. The alltrue
    # is wrapped in try() rather than can() because can() tests only that the
    # expression evaluates, which alltrue always does: an entry whose name is
    # "" must fail this as surely as an entry with no name at all. The
    # tostring() comparison holds only for an actual string, since Terraform's
    # == never converts across types: a bare number would satisfy a length
    # check and then render as a numeric name the CNPG schema rejects.
    condition     = var.cnpg_plugins == null ? true : can(var.cnpg_plugins[0]) && try(alltrue([for p in var.cnpg_plugins : p.name == tostring(p.name) && length(p.name) > 0]), false)
    error_message = "cnpg_plugins must be null or a non-empty list of objects that each carry a name (e.g. \"barman-cloud.cloudnative-pg.io\"). The name is what the operator dispatches on, so an entry without one reaches nothing."
  }
}
...
variable "n8n_main_strategy" {
  ...
  validation {
    ...
    error_message = "n8n_main_strategy must be one of: Recreate, RollingUpdate. Recreate waits for the old main to terminate before starting the new one. RollingUpdate renders maxSurge = 0, which is as close as that controller gets, but still starts the new pod while the old one is draining. Kubernetes' own surging default is not offered at all, because it starts the new main before the old one is even asked to stop."
  }
}
