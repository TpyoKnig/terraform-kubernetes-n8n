...
variable "cnpg_plugins" {
  ...
  validation {
    ...
    # "" must fail this as surely as an entry with no name at all.
    condition     = var.cnpg_plugins == null ? true : can(var.cnpg_plugins[0]) && try(alltrue([for p in var.cnpg_plugins : length(tostring(p.name)) > 0]), false)
