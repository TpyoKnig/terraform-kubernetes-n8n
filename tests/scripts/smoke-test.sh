#!/usr/bin/env bash
# smoke-test.sh, post-deployment smoke test for terraform-kubernetes-n8n.
#
# The post-apply half of this module's verification: it asserts what a plan
# cannot. Terraform reports success once the Helm release and the CNPG Cluster
# CR are applied, which says nothing about whether an operator reconciled them,
# whether the ingress controller picked up the routes, or whether a workflow
# can actually execute. This checks all of that against a live deployment.
#
# It auto-detects the topology by probing the namespace. Queue mode (separate
# worker and webhook-processor pools) is what this module deploys by default,
# so the multi path is the usual one: pod health, queue mode, Redis, the KEDA
# ScaledObject, HTTPS, API, and an end-to-end execution.
#
# Usage:
...
#   TERRAFORM_DIR=examples/homelab ./tests/scripts/smoke-test.sh
#
...
if command -v terraform &>/dev/null && [[ -f "$TERRAFORM_DIR/terraform.tfstate" ]]; then
  ...
  # Switch kubectl context to the cluster this deployment sits on. The examples
  # emit kubectl_config_command for exactly this; the module itself does not,
  # since it does not own the cluster, so a caller's own root may not have it.
  ...
  # Print it either way. Where no command was available, running the whole suite
  # against the wrong cluster is otherwise silent, and every check below would
  # be reporting on someone else's deployment.
  echo -e "\033[0;36m↳\033[0m  context   = $(kubectl config current-context 2>/dev/null || echo '<none>')"
...
# Expected minimum replica counts. Exactly one main: leader election among
# mains is a licensed feature this module does not carry, so a second main
# would be a second leader. Queue mode carries the load through the worker and
# webhook-processor pools, which are the ones that scale.
MAIN_MIN=1
WORKER_MIN=1
WEBHOOK_MIN=2
...
# A working Python, used to read ids out of the n8n API's JSON responses.
#
# Resolved by running one, not by checking PATH: on Windows, `python3` is a
# Microsoft Store stub that resolves fine and then prints an install notice to
# stdout instead of executing. Combined with the `|| echo ""` guards around
# each parse, that yielded an empty workflow id and a downstream HTTP 404 on
# activation, a confusing failure a long way from its cause. Found on exactly
# that box.
PY_JSON=""
for _candidate in python3 python py; do
  if command -v "$_candidate" &>/dev/null && echo '{"ok":1}' | "$_candidate" -c 'import sys,json;json.load(sys.stdin)' &>/dev/null; then
    PY_JSON="$_candidate"
    break
  fi
done
...
if [[ -z "$PY_JSON" ]]; then
  warn "No working Python found - the API and execution tests will be skipped"
  warn "  Tried: python3, python, py"
fi
...
if [[ "$DEPLOY_MODE" == "multi" ]]; then
  ...
  info "Checks: queue mode, HPA/KEDA, Redis, single-main topology"
else
...
# ── Single-main topology ─────────────────────────────────────────

header "Single-Main Topology"

# Asserted, not assumed. A second main pod without leader election means two
# pods both believing they own the schedule triggers and the waiting
# executions: duplicate runs, with nothing anywhere reporting an error.
main_replicas=$(kubectl get deployment n8n-main -n "$NAMESPACE"   -o jsonpath='{.spec.replicas}' 2>/dev/null || echo "")
if [[ "$main_replicas" == "1" ]]; then
  pass "n8n-main is a single replica"
else
  fail "n8n-main has ${main_replicas:-<unreadable>} replicas, expected exactly 1"
fi

main_pod=$(kubectl get pods -n "$NAMESPACE"   -l "app.kubernetes.io/component=main"   --field-selector=status.phase=Running   -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)

if [[ -n "$main_pod" ]]; then
  multi_main=$(kubectl exec "$main_pod" -n "$NAMESPACE" -c n8n-main     -- printenv N8N_MULTI_MAIN_SETUP_ENABLED 2>/dev/null || echo "")
  if [[ "$multi_main" == "true" ]]; then
    fail "N8N_MULTI_MAIN_SETUP_ENABLED=true on the main pod, but this module renders no multi-main values"
  else
    pass "N8N_MULTI_MAIN_SETUP_ENABLED is unset on main pods (got: '${multi_main:-<unset>}')"
  fi

  licence_env=$(kubectl exec "$main_pod" -n "$NAMESPACE" -c n8n-main     -- sh -c 'printenv | grep -c "^N8N_LICENSE_" || true' 2>/dev/null || echo "0")
  if [[ "${licence_env:-0}" == "0" ]]; then
    pass "No N8N_LICENSE_* variables on the main pod - Community edition"
  else
    fail "${licence_env} N8N_LICENSE_* variable(s) on the main pod; this module carries no licence"
  fi
else
  warn "No running main pod found to check the single-main topology"
fi
...
# No main HPA by design: hpa.main.enabled is hardcoded false, because scaling
# the main Deployment is only safe under multi-main.
if kubectl get hpa n8n-main -n "$NAMESPACE" &>/dev/null 2>&1; then
  fail "An HPA exists for n8n-main; the module disables it, so something else created it"
else
  pass "No n8n-main HPA, as designed (the main Deployment does not scale)"
fi
# Two possible names. Off the KEDA path the chart renders its own
# n8n-webhook-processor HPA; on it, the chart suppresses that one and the
# module renders n8n-webhook-processor-supplementary in its place. Exactly one
# should exist, and which one is not the point: that the pool has an
# autoscaler at all is.
if kubectl get hpa n8n-webhook-processor -n "$NAMESPACE" &>/dev/null 2>&1; then
  check_hpa "n8n-webhook-processor" "Webhook processor (chart)"
elif kubectl get hpa n8n-webhook-processor-supplementary -n "$NAMESPACE" &>/dev/null 2>&1; then
  check_hpa "n8n-webhook-processor-supplementary" "Webhook processor (module, KEDA path)"
else
  fail "No HPA for the webhook processor. The chart suppresses its own whenever keda.enabled is true, so the module must supply one."
fi
...
# The module installs no CSI driver and no StorageClass, both are caller
# prerequisites. Assert only that a class exists for the PVCs to bind against.
if [[ -n "$default_sc" ]]; then
  pass "Default StorageClass is '$default_sc'"
else
  warn "No default StorageClass - CNPG and Valkey PVCs will stay Pending unless cnpg_storage_class / valkey_storage_class name one explicitly"
fi

# ── In-cluster backing services ───────────────────────────────────────────────
# A managed database is someone else's problem to keep healthy. These are not:
# a CNPG cluster that never reaches a ready instance, or a Valkey pod that never
# binds its PVC, is this module's problem and the most likely way a deployment
# fails.

header "In-Cluster Backing Services"

cnpg_cluster=$(kubectl get cluster.postgresql.cnpg.io -n "$NAMESPACE"   -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)

if [[ -z "$cnpg_cluster" ]]; then
  info "No CNPG Cluster in $NAMESPACE - postgres_backend is not cnpg, skipping"
else
  cnpg_ready=$(kubectl get cluster.postgresql.cnpg.io "$cnpg_cluster" -n "$NAMESPACE"     -o jsonpath='{.status.readyInstances}' 2>/dev/null || echo "0")
  cnpg_want=$(kubectl get cluster.postgresql.cnpg.io "$cnpg_cluster" -n "$NAMESPACE"     -o jsonpath='{.spec.instances}' 2>/dev/null || echo "0")
  if [[ "${cnpg_ready:-0}" -ge 1 && "${cnpg_ready:-0}" == "${cnpg_want:-0}" ]]; then
    pass "CNPG cluster '$cnpg_cluster' has $cnpg_ready/$cnpg_want instances ready"
  else
    fail "CNPG cluster '$cnpg_cluster' has ${cnpg_ready:-0}/${cnpg_want:-0} instances ready - kubectl describe cluster $cnpg_cluster -n $NAMESPACE"
  fi

  # The app-user Secret CNPG generates is what the chart reads for its database
  # password. Its absence is the failure mode behind a pod stuck in
  # CreateContainerConfigError.
  if kubectl get secret "${cnpg_cluster}-app" -n "$NAMESPACE" &>/dev/null; then
    pass "CNPG app-user Secret '${cnpg_cluster}-app' exists"
  else
    fail "CNPG app-user Secret '${cnpg_cluster}-app' missing - the n8n pods have no database password to mount"
  fi
fi

valkey_running=$(kubectl get pods -n "$NAMESPACE" -l app.kubernetes.io/name=valkey   --no-headers 2>/dev/null | grep -c "Running" || true)
if [[ "${valkey_running:-0}" -gt 0 ]]; then
  pass "Valkey is running in $NAMESPACE (${valkey_running} pod(s))"
else
  info "No running Valkey pods in $NAMESPACE - redis_backend is not valkey, skipping"
fi

# The chart's CPU HPA and a KEDA ScaledObject must never both own the worker
# Deployment. Assert on whichever is present rather than requiring KEDA, since
# queue-depth scaling is opt-in on this backend.
# Custom columns rather than a jsonpath carrying an embedded newline escape:
# that escape is easy to break when this file is edited programmatically, and
# the original || true then turned a broken query into a confident 'no
# ScaledObject found'. Distinguish the three outcomes rather than collapsing a
# failed lookup into an empty result.
if ! kubectl get crd scaledobjects.keda.sh &>/dev/null; then
  info "KEDA CRDs are not installed in this cluster - workers scale on the chart's CPU HPA"
else
  scaledobjects=$(kubectl get scaledobject -n "$NAMESPACE" --no-headers \
    -o custom-columns=NAME:.metadata.name,READY:'.status.conditions[?(@.type=="Ready")].status' 2>/dev/null)
  so_rc=$?
  if [[ "$so_rc" -ne 0 ]]; then
    warn "Could not list ScaledObjects in $NAMESPACE (kubectl exited $so_rc)"
  elif [[ -z "${scaledobjects//[[:space:]]/}" ]]; then
    info "No KEDA ScaledObject in $NAMESPACE - workers scale on the chart's CPU HPA (k8s_keda_installed = false)"
  else
    while read -r so_name so_ready; do
      [[ -z "$so_name" ]] && continue
      if [[ "$so_ready" == "True" ]]; then
        pass "KEDA ScaledObject '$so_name' is Ready"
      else
        fail "KEDA ScaledObject '$so_name' is not Ready (${so_ready:-unknown}) - k8s_keda_installed is true but is the KEDA operator running?"
      fi
    done <<< "$scaledobjects"
  fi
fi
...
# The chart renders its own Ingresses (n8n-main and n8n-webhook-processor) and
# the module names none of its own, so collect paths across every Ingress in the
# namespace rather than hardcoding one name. That also covers
# create_ingress = false, where the Ingresses are the caller's and named
# whatever the caller chose.
# The .items[*] wrapper is load-bearing: listing without a resource name returns
# a List, and a jsonpath rooted at .spec silently matches nothing there. That
# reads identically to a namespace with no Ingress at all, so the check would
# skip itself and report success on a deployment whose webhook routes were
# wrong.
ingress_paths=$(kubectl get ingress -n "$NAMESPACE" \
  -o jsonpath='{range .items[*]}{range .spec.rules[*].http.paths[*]}{.path}{"="}{.backend.service.name}{"\n"}{end}{end}' \
  2>/dev/null || true)

if [[ -z "$ingress_paths" ]]; then
  skip "Ingress routing (no Ingress found in namespace '$NAMESPACE')"
...
if [[ -z "$N8N_URL" ]]; then
  ...
  if [[ "$healthz_status" == "200" ]]; then
    ...
    # An unreachable host is the expected result, not a failure, when the editor
    # is deliberately private: examples/homelab-split-ingress puts it on a
    # LAN-only hostname, and a homelab ingress commonly sits on an RFC1918
    # address that nothing outside the network can reach. Distinguish the two
    # cases by what the name resolves to, so a genuine outage still fails
    # loudly.
    n8n_host="${N8N_URL#*://}"; n8n_host="${n8n_host%%/*}"
    # `|| true`: under `set -euo pipefail`, grep exits 1 when the name does not
    # resolve, which is itself one of the cases this branch exists to report (a
    # private record that has not propagated to this host). Without the guard
    # the pipeline aborts the whole script before the summary prints.
    ...
    if [[ "$resolved" =~ ^10\. || "$resolved" =~ ^192\.168\. || "$resolved" =~ ^172\.(1[6-9]|2[0-9]|3[01])\. ]]; then
      ...
      info "This is expected for a LAN-only editor hostname, e.g."
      info "examples/homelab-split-ingress. Re-run from inside that network,"
...
if [[ -z "$N8N_URL" || -z "$N8N_API_KEY" || -z "$PY_JSON" ]]; then
  skip "Workflow execution test (requires N8N_URL, N8N_API_KEY and a working Python)"
else
  ...
  if [[ "$create_status" != "200" ]]; then
    ...
    workflow_id=$(echo "$create_body" | "$PY_JSON" -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || echo "")
      ...
      if [[ "$trigger_status" =~ ^2 ]]; then
        ...
        for i in $(seq 1 30); do
          ...
          exec_state=$(curl -sk \
            ...
            | "$PY_JSON" -c "import sys,json; d=json.load(sys.stdin); execs=d.get('data',[]); print(execs[0]['status'] if execs else 'pending')" 2>/dev/null \
            || echo "unknown")
...
elif [[ -z "$N8N_URL" || -z "$N8N_API_KEY" || -z "$PY_JSON" ]]; then
  skip "Load scaling test (requires N8N_URL, N8N_API_KEY and a working Python)"
else
  SCALER_MODE=""
...
EOF
    ...
    if [[ "$load_create_status" != "200" ]]; then
      ...
      load_workflow_id=$(echo "$load_create_body" \
        | "$PY_JSON" -c "import sys,json; print(json.load(sys.stdin)['id'])" 2>/dev/null || echo "")
