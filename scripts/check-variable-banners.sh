...
# builds even under a UTF-8 locale, and every banner then reads as absent.
...
for file in "${FILES[@]}"; do
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    lineno=$((lineno + 1))
    # Strip a trailing CR so a CRLF checkout (git core.autocrlf on Windows)
    # does not defeat the $-anchored strict banner regex below.
    line=${line%$'\r'}
